...
function GbetaE_eval_samples(N::Int64, num_taken::Int64, mu::Float64, sigma_sq::Float64, beta::Float64, num_samples::Int64)::Array{Float64,2}
        ...
        samples num_samples instances of num_taken (out of total of N) Gaussian Beta Ensemble points (inverse temperature = power of Vandermonde interaction = beta) from the distribution with univariate potential 
            
                exp( -(x - mu)^2 / (2 sigma_sq) ) 

        (mean mu, variance sigma_sq normal distribution)
    
        uses the Edelman--Dumitriu tridiagonal method described in Theorem 2.1 of the paper 'Fast sampling from beta-ensembles' by Gautier--Bardenet--Valko arXiv:2003.02344 [stat.CO]
        ...
        output: Lambda = N x num_samples matrix, each of it's columns an independent GbetaE sample
        ...
        Lambda = zeros(num_taken, num_samples) # each column of Lambda will be one vector of num_taken GbetaE points ordered ascendingly 
...
function GUE_eval_samples(N::Int64, num_taken::Int64, num_samples::Int64)::Array{Float64,2}
    ...
    samples num_samples instances of num_taken (out of total of N) Gaussian Unitary Ensemble (GUE) points (inverse temperature = power of Vandermonde interaction = beta) from the distribution with univariate potential 
    ...
    output: Lambda = N x num_samples matrix, each of it's columns an independent GUE sample
    ...
    Lambda = zeros(num_taken, num_samples) # each column of Lambda will be one vector of num_taken GUE points ordered ascendingly 
...
function LbetaE_eval_samples(N::Int64, num_taken::Int64, alpha::Float64, theta::Float64, beta::Float64, num_samples::Int64)::Array{Float64,2}
        ...
        samples num_samples instances of num_taken (out of total of N) Laguerre Beta Ensemble points (inverse temperature = power of Vandermonde interaction = beta) from the distribution with univariate potential 
            
                x^( alpha - 1 ) * exp( -x / theta ) --- NOTE the -1
    
        uses the Edelman--Dumitriu tridiagonal method described in Theorem 2.2 of the paper 'Fast sampling from beta-ensembles' by Gautier--Bardenet--Valko arXiv:2003.02344 [stat.CO]
        ...
        output: Lambda = N x num_samples matrix, each of it's columns an independent LbetaE sample
        ...
        Lambda = zeros(num_taken, num_samples) # each column of Lambda will be one vector of num_taken LbetaE points ordered ascendingly 
...
#function LUE_eval_samples(N, alpha, num_samples) would also do
    """

    samples num_samples instances of num_taken (out of max N) points from the N x N LUE (Laguerre Unitary Ensemble, beta = 2) having distribution with univariate potential 
    ...
    output: Lambda = N x num_samples matrix, each of it's columns an independent LUE sample
    ...
    Lambda = zeros(num_taken, num_samples) # each column of Lambda will be one vector of num_taken LUE points ordered ascendingly 
...
function JbetaE_eval_samples(N::Int64, num_taken::Int64, a::Float64, b::Float64, beta::Float64, num_samples::Int64)::Array{Float64,2}
    ...
    samples num_samples instances of num_taken (out of total of N) Jacobi Beta Ensemble points (inverse temperature = power of Vandermonde interaction = beta) from the distribution with univariate potential 
        
            x^( a - 1 ) * ( 1 - x )^( b - 1 ) --- NOTE the -1 in the exponents

    uses the Killip--Nenciu tridiagonal method described in Theorem 2.3 of the paper 'Fast sampling from beta-ensembles' by Gautier--Bardenet--Valko arXiv:2003.02344 [stat.CO]
    ...
    output: Lambda = N x num_samples matrix, each of it's columns an independent JbetaE sample
    ...
    Lambda = zeros(num_taken, num_samples) # each column of Lambda will be one vector of num_taken JbetaE points ordered ascendingly 
...
function JUE_eval_samples(N::Int64, num_taken::Int64, a::Float64, b::Float64, num_samples::Int64)::Array{Float64,2}
    ...
    samples num_samples instances of num_taken (out of total of N) Jacobi Unitary Ensemble points (inverse temperature = power of Vandermonde interaction = 2) from the distribution with univariate potential 
    ...
    output: Lambda = N x num_samples matrix, each of it's columns an independent JUE sample
    ...
    Lambda = zeros(num_taken, num_samples) # each column of Lambda will be one vector of num_taken JUE points ordered ascendingly 
